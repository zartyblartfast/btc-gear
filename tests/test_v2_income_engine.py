...
def config(
    *,
    annual_income_target: float = 50_000.0,
    ...
    return IncomeConfig(
        ...
        annual_income_target=annual_income_target,
...
def test_income_year_funds_requested_income_up_to_safe_capacity() -> None:
    prior = income_setup(config(annual_income_target=40_000.0))
    row = income_year(config(annual_income_target=40_000.0), prior, PricePoint(year=1, btc_price=60_000.0))
...
def test_income_year_partially_funds_income_when_capacity_is_limited() -> None:
    prior = income_setup(config(annual_income_target=50_000.0))
    row = income_year(config(annual_income_target=50_000.0), prior, PricePoint(year=1, btc_price=60_000.0))
...
def test_income_year_uses_liquidation_buffer_when_it_is_stricter_than_ltv_ceiling() -> None:
    cfg = config(annual_income_target=50_000.0, income_ltv_ceiling=0.50)
...
def test_income_year_capitalizes_interest_before_computing_capacity() -> None:
    ...
    row = income_year(config(annual_income_target=10_000.0), prior, PricePoint(year=2, btc_price=60_000.0))
...
def test_income_year_paid_external_interest_does_not_reduce_capacity() -> None:
    cfg = config(annual_income_target=2_000.0, interest_treatment=InterestTreatment.PAID_EXTERNALLY)
    prior = IncomeRow(
        year=1,
        btc_price=60_000.0,
        collateral_btc=2.0,
...
def test_income_year_zero_debt_and_zero_income_remains_safe() -> None:
    cfg = config(annual_income_target=0.0)
    prior = income_setup(cfg)
    row = income_year(cfg, prior, PricePoint(year=1, btc_price=60_000.0))
